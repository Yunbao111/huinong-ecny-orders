...
import { ArrowRight, BadgeCheck, CheckCheck, ClipboardCheck, Clock3, Fingerprint, Lock, RefreshCw, ShieldCheck, UserRound, WalletCards } from 'lucide-react';
...
type Panel = 'delivery' | 'inspection' | 'confirm' | null;

export default function OrdersPage() {
  const { role, fulfillment, payment, expectedFen, paymentFen, recordDelivery, passInspection, confirmAcceptance, retryPayment, resetDemo } = useDemo();
  ...
  const currentIndex = fulfillment === 'accepted' ? 1 : fulfillment === 'delivered' ? 2 : fulfillment === 'inspection_passed' ? (payment === 'awaiting_confirm' ? 3 : 4) : 5;
  const progressSteps = useMemo(() => [
    ...
    { label: '农户确认', detail: currentIndex >= 4 ? '已确认验收结果' : '等待农户确认' },
    { label: '仿真付款', detail: payment === 'paid' ? '已完成' : payment === 'processing' ? '正在处理' : payment === 'retry_pending' ? '等待重试' : '确认后触发' },
  ].map((step, index) => ({ ...step, state: index < currentIndex ? 'done' : index === currentIndex ? 'current' : 'next' })), [currentIndex, fulfillment, payment]);

  const status = fulfillment === 'accepted' ? '已接单' : fulfillment === 'delivered' ? '待验收' : fulfillment === 'inspection_passed' && payment === 'awaiting_confirm' ? '待农户确认' : payment === 'paid' ? '已完成' : payment === 'retry_pending' ? '付款待重试' : '仿真付款中';
  ...
  function confirmAccept() {
    if (confirmAcceptance()) setPanel(null);
  }

  return (
    <main className="mx-auto max-w-7xl px-4 pb-28 pt-6 sm:px-6 lg:px-8 lg:pb-14">
      <section className="page-heading">
        <div><p className="section-kicker">我的订单</p><h1>把交货、验收、确认和数币付款连在一起</h1><p>当前演示身份：<strong>{roles[role]}</strong>。每种身份只能办理自己负责的步骤，农户拥有确认验收结果的权利。</p></div>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[1fr_310px]">
        <div>
            ...
            <div className="grid lg:grid-cols-[290px_1fr]">
              ...
              <div className="p-5 sm:p-7">
                ...
                <CardContent className="px-0">
                  ...
                  <div className="mt-7 grid grid-cols-5 gap-1" aria-label="订单进度">
                  ...
                  <OrderAction role={role} fulfillment={fulfillment} payment={payment} onDelivery={() => setPanel('delivery')} onInspection={() => setPanel('inspection')} onConfirm={() => setPanel('confirm')} onRetry={retryPayment} />
                </CardContent>
              </div>
            </div>
          </Card>
          ...
          {panel === 'inspection' && <ActionPanel title="苹果现场验收" description="系统按照合格重量和冻结的订单单价计算仿真付款金额。" onClose={() => setPanel(null)}><div className="grid gap-4 sm:grid-cols-2"><div><Label className="mb-2 text-base font-bold" htmlFor="delivered">实际送达（千克）</Label><Input id="delivered" className="h-12 text-lg" value="1980" disabled /></div><div><Label className="mb-2 text-base font-bold" htmlFor="accepted">验收合格（千克）</Label><Input id="accepted" className="h-12 text-lg" inputMode="numeric" value={acceptedWeight} onChange={(event) => setAcceptedWeight(event.target.value)} /></div></div><div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-bold text-amber-800">系统计算的仿真付款金额</p><p className="mt-1 text-3xl font-black text-emerald-800">{formatMoney(amountFen)}</p><p className="mt-2 text-sm text-slate-600">合格重量 × 5.20 元/千克；金额按“分”精确计算。</p></div><Button type="button" onClick={confirmInspection} className="mt-5 h-13 w-full rounded-xl text-lg font-black" disabled={role !== 'inspector' || fulfillment !== 'delivered' || !exactDemoAmount}>验收通过，等待农户确认</Button>{!exactDemoAmount && <p role="alert" className="mt-3 text-sm font-bold text-red-700">本次演示合格重量应为 1,950 千克，请核对后再提交。</p>}</ActionPanel>}

          {panel === 'confirm' && <ActionPanel title="确认验收结果" description="验收合格重量与金额已生成，请农户核对后确认；确认即触发数币条件支付。" onClose={() => setPanel(null)}><div className="grid gap-3 rounded-2xl bg-slate-50 p-4 sm:grid-cols-3"><OrderFact label="验收合格" value="1,950 千克" /><OrderFact label="结算单价" value="5.20 元/千克" /><OrderFact label="应收货款" value={formatMoney(paymentFen)} /></div><div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-sm font-bold text-emerald-800">确认即触发数币条件支付，货款秒级划入你的钱包，不再取决于采购方何时打款。</p></div><Button type="button" onClick={confirmAccept} className="mt-5 h-13 w-full rounded-xl text-lg font-black" disabled={role !== 'farmer' || fulfillment !== 'inspection_passed' || payment !== 'awaiting_confirm'}><CheckCheck />确认无误，同意收款</Button></ActionPanel>}

          <MoneyStage payment={payment} />
          <EvidenceCard fulfillment={fulfillment} payment={payment} />
        </div>

        <aside className="space-y-5">
          <Card className="rounded-[22px] border-0 bg-[var(--forest)] text-white ring-0"><CardHeader><CardDescription className="text-base text-emerald-100">为什么验收后还要农户确认？</CardDescription><CardTitle className="mt-2 text-xl font-black">把“说了算”还给农户</CardTitle></CardHeader><CardContent><p className="text-base leading-7 text-emerald-50">过去验收是对方单方面说了算。现在验收结果要农户确认，货款才自动结算——让农户在交易里第一次有了确认权和话语权。</p><SafeLink href="/digital-rmb#platform-fit" className="mt-4 inline-flex items-center gap-2 font-black text-amber-200 underline underline-offset-4">了解数币如何赋能 <ArrowRight className="size-5" /></SafeLink></CardContent></Card>
          <Card className="rounded-[22px] border-0 bg-white ring-1 ring-emerald-950/8"><CardHeader><CardTitle className="flex items-center gap-2 text-xl font-black"><Clock3 className="size-6 text-[var(--leaf)]" />办理记录</CardTitle></CardHeader><CardContent className="space-y-4"><AuditRow done title="农户接受订单" detail="2026年8月20日・条款冻结" /><AuditRow done={fulfillment !== 'accepted'} title="农户登记交货" detail={fulfillment === 'accepted' ? '尚未办理' : '1,980 千克'} /><AuditRow done={currentIndex >= 3} title="验收员确认合格量" detail={currentIndex >= 3 ? '合格 1,950 千克' : '尚未办理'} /><AuditRow done={currentIndex >= 4} title="农户确认验收结果" detail={currentIndex >= 4 ? '已确认' : '尚未办理'} /><AuditRow done={payment === 'paid'} title="仿真付款记录" detail={payment === 'paid' ? '10,140.00 元' : payment === 'processing' ? '正在处理' : payment === 'retry_pending' ? '等待管理员重试' : '尚未触发'} /></CardContent></Card>
...
function EvidenceCard({ fulfillment, payment }: { fulfillment: string; payment: string }) {
  const events = [
    { title: '订单确认', time: '2026-08-20 10:24', hash: '0x8f2a1c7e', done: true },
    { title: '交货登记', time: '2026-09-06 08:15', hash: '0x4b7d90c3', done: fulfillment !== 'accepted' },
    { title: '验收合格', time: '2026-09-07 14:02', hash: '0xc3e5a6f8', done: fulfillment !== 'accepted' && fulfillment !== 'delivered' },
    { title: '数币付款', time: '2026-09-07 14:03', hash: '0x19d4f26b', done: payment === 'paid' },
  ];
  return (
    <Card className="mt-5 rounded-[22px] border-0 bg-white ring-1 ring-emerald-950/8">
      <CardHeader><CardTitle className="flex items-center gap-2 text-xl font-black"><Fingerprint className="size-6 text-[var(--leaf)]" />三单合一存证回单</CardTitle><CardDescription className="mt-2 text-base">订单、验收、付款每步生成哈希 + 时间戳，农户与采购方各持一份，可对账、可举证。</CardDescription></CardHeader>
      <CardContent className="space-y-3">
        {events.map((e) => (
          <div key={e.title} className={`flex items-center gap-3 rounded-2xl border p-4 ${e.done ? 'border-emerald-100 bg-emerald-50/50' : 'border-slate-200 bg-slate-50'}`}>
            <span className={`grid size-8 shrink-0 place-items-center rounded-full ${e.done ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>{e.done ? <BadgeCheck className="size-5" /> : <Clock3 className="size-5" />}</span>
            <div className="min-w-0 flex-1"><p className="font-black">{e.title}</p><p className="mt-0.5 text-sm text-slate-500">{e.time}</p></div>
            <code className="rounded-lg bg-slate-100 px-2 py-1 text-xs font-bold text-slate-600">{e.done ? e.hash : '—'}</code>
          </div>
        ))}
        <p className="text-sm leading-6 text-slate-500">存证为仿真示意，不代表真实区块链或司法存证；真实接入需通过运营机构完成。</p>
      </CardContent>
    </Card>
  );
}
